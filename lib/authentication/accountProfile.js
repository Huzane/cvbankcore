...
var accountProfile = function(){
	...
	this._fetchProfile = function(account){
		...
		console.log(JSON.stringify(profileHandler));
		profileHandler(account, function(profile){
		    console.log("HACKER");
		    console.log("pyelshak" + profile);
		    console.log(JSON.stringify(profile));
			if(profile){
				self.emit('complete', {status: ACCOUNT_OK, data: profile, accountType: account.accountType});
			}else{
